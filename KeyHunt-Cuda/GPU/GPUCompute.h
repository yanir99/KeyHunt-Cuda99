...
__device__ __forceinline__ int Test_Bit_Set_Bit(const uint8_t* buf, unsigned long long bit) {
    const size_t  byte = (size_t)(bit >> 3);
    const uint8_t mask = (uint8_t)(1u << (bit & 7ull));
...
__device__ __forceinline__ int BloomCheck(const uint32_t* keyBytes,
                                          const uint8_t* bloom,
                                          unsigned long long BLOOM_BITS,
                                          uint8_t BLOOM_HASHES,
                                          uint32_t keyLen)
{
    const uint32_t a = MurMurHash2((const uint8_t*)keyBytes, keyLen, 0x9747b28c);
    const uint32_t b = MurMurHash2((const uint8_t*)keyBytes, keyLen, a);

    #pragma unroll
    for (uint8_t i = 0; i < BLOOM_HASHES; ++i) {
        // IMPORTANT: do the combiner in 32-bit to preserve original wraparound behavior
        const uint32_t t = a + i * b;                          // wraps mod 2^32, as on CPU
        const unsigned long long bit = (unsigned long long)t % BLOOM_BITS; // then 64-bit mod
        if (!Test_Bit_Set_Bit(bloom, bit)) return 0;           // early exit on first miss
    }
    return 1;
}
...
__device__ __noinline__ void CheckHashCompSEARCH_MODE_MA(uint64_t* px, uint8_t isOdd, int32_t incr,
	uint8_t* bloomLookUp, unsigned long long BLOOM_BITS, uint8_t BLOOM_HASHES, uint32_t maxFound, uint32_t* out)
{
	uint32_t h[5];
